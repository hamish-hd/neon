...
df4.loc[:, 'subplotID'] = df4['subplotID'].str.replace(r'_25_unknown', '_100')
...
df4.loc[:, 'subplotID'] = df4['subplotID'].str.replace(r'_10_unknown', '_100')
...
for root, dirs, files in os.walk(struct_plant_dir2):
    for file in files:
        if file.endswith('.csv') and 'chm_height' in file and 'TALL' in file:
            file_path = os.path.join(root, file)
            df = pd.read_csv(file_path)
            print('ek ka shape :',df.shape)
            print(file_path)
...
ax.set_title('AGB (2004) vs CHM 95 (TALL)')
