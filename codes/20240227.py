...
#%% Remove outliers before fitting curve
...
outlier_threshold = 3
v1 = v1[np.abs(v1['AGB'] - v1['AGB'].mean()) < outlier_threshold * v1['AGB'].std()]

# Plot scatter plot after removing outliers
plt.figure(figsize=(8, 8), dpi=300)
plt.scatter(v1['height'], v1['AGB'], label='Scatter Plot (Outliers Removed)')
plt.xlabel('Height')
plt.ylabel('AGB')
...
# Initialize best parameters and covariance matrix
best_params, cov = minimize(curve_function, [1, 1], args=(v1['height'], v1['AGB'])).x

# Test the model on the testing set
predictions = best_params[0] * np.power(v1['height'], best_params[1])
...
# Plot best-fit curve
plt.figure(figsize=(8, 8), dpi=300)
plt.scatter(v1['height'], v1['AGB'], label='Data (Outliers Removed)')
x_curve = np.linspace(0, 50, 100)
y_curve = best_params[0] * np.power(x_curve, best_params[1])
plt.plot(x_curve, y_curve, color='red', label=f'Best Fit: {best_params[0]:.2f} * x**{best_params[1]:.2f}\nRMSE: {fit_error:.2f}\nR-squared: {r2:.2f}')
plt.ylim(0, 400)
plt.xlabel('CHM Height (chm_mean)')
plt.ylabel('AGB 2004')
plt.title('Scatter Plot and Best Fit Curve (Outliers Removed)')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.show()
